...
class Rob_env(Environment):
	def __init__(self, walls = {}):
		# Walls is a set of line segments where each line segnment is
...
class Rob_body(Environment):
	def __init__(self, env, init_pos = (0, 0, 90)):
		...
		# direction is in degrees; 0 is to right, 90 is straight up,
		# etc.
		self.env = env
		self.rob_x, self.rob_y, self.rob_dir = init_pos
		self.turning_angle = 18 # degrees that a left makes
		self.whisker_length = 6 # length of a whisker
...
def line_segments_intersect(linea, lineb):
	...
	# A line segment is represented as a pair of points. A point is
	# represented as a (x,y) pair.
	((x0a,y0a), (x1a,y1b)) = linea
