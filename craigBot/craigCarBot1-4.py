...
# Runs the bot.
# @param m, the Tkinter object.
# @param make, the make of the vehicle.
# @param model, the model of the vehicle.
# @oaram yearRange1, the min year range.
# @param yearRange2, the max year range.
...
def runBot(m, make, model, yearRange1, yearRange2, email):
    ...
    while True:
        ...
        if dayCondition and time_of_day == "12:00:00":
            ...
            # If there was an entry for that day, skip. # Not likely
            # due to current runtime of program.
...
# @param newYearRange, the year range provided by the customer.
# @param make, make of the vehicle.
# @param model, model of the vehicle.
# @param email, email of the customer.
# @param curntDate, the current date of the search.
# @param rsltNM, the number of results on that page.
# @return returns nothing.
...
def scanEntries(firefox_driver, data):
    ...
    for i in range(len(entriesList)):
        ...
        enc = 'utf-8'
        title = firefox_driver.find_element_by_id(titleStr).text
        title = title.encode(enc)
        #prStr = "price"
        ...
        #wTitle = firefox_driver.find_element_
        #       by_class_name(pTTxt).text.encode(utf-8)
