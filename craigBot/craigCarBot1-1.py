...
# Chagnes: Removed cluttered code.
# Python 2.7
# Windows 10
...
def runCarSearch(newYearRange, make, model, email, currentDate):
	...
	while topNum != resultsNum and int(resultsNum) != 0:
		...
		# Jump to next page.
		firefox_driver.implicitly_wait(5)
		#nextPage = firefox_driver.find_element_by_class_name("button next")
		nextPage = firefox_driver.find_element_by_xpath("//a[@class='button next']")
		#print nextPage
		nextPage.click()

	# Write data to file.
	#writeFile(data, resultsName, currentDate)

	# Send user email with data
	#sendEmail(email, data, server)
...
def scanEntries(firefox_driver, data):
	...
	for i in range(len(entriesList)):
		...
		title = str(firefox_driver.find_element_by_id(titleStr).text)
		# Store data to tuple.
...
def writeFile(data, resultsName, timestamp):
	...
	for item in data:
		...
		# 	Duplicates will not be rewritten.
		#if item in lines:
		#	data.remove(item)
		#	continue
		#else:
